...
Now that we have learnt to visualize the data, we will look at the next step in the process - analysis of data. It is the process of systematically applying statistical/logical techniques to describe and evaluate data. We will explore the concept of Logistical Regression using different models to analyze the data, while also learning to understand the output. 
...
data(iris)
summary(iris)
sub_iris <- subset(iris,Species %in% c('setosa','virginica'))
sub_iris
```
...
To start the modeling process, we include one explanatory variable - Age. 

```{r}
mod1 <- glm(Species ~ Sepal.Length, data = sub_iris, family = "binomial")
```
...
```{r}
mod2 <- glm(Species ~ Sepal.Length + Sepal.Width + Petal.Length, data = sub_iris, family = "binomial")
summary(mod2)
```
...
```{r}
mod3 <- glm(Species ~ Sepal.Length + Sepal.Width + Petal.Length + Petal.Width, data = sub_iris, family = "binomial")
summary(mod3)
```
