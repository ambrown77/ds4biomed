...
1. Understand the file system cataloguing in computer
2. Recognize different types of file formats and operations that can be used 
...
First, we need to load a package to make loading data sets easier.
...
::::: {.rBlock}
###### R {-}
First, we will load the `tidyverse` packages using the `library` function.

```{r}
library(tidyverse)
```

The first time you load the `tidyverse` library,
...
If you run a piece of code and end up with an `could not find function` error,
...
Let the below figures represent a `ds4es` folder on your Desktop on Windows and Mac, respectively.
...
The way we can **relatively** reference the previous folder is with two dots, `..`, `../data/medicaldata_tumorgrowth.csv`
...
Let us say we are currently in the `chart_review/analysis` folder, working on our `demographics.R` as denoted by the `#`.
...
So if you want to use `\` for file paths in Windows, you will have to use two backslashes, e.g., `..\\data\\patients.csv\\`.
...
Now that we know how to find our files, it is time to load up our first data set.
...
```{r}
read_csv("data/crop_recommendation.csv")
```
:::::
...
- If the above code returns a `does not exist in current working directory`,
  make sure the working directory, which it lists, is your expected "starting point" (i.e., working directory),
  and make sure the file path is spelled correctly.

`read_csv` will show the columns that were read in
...
We do not want to re-load the data set every time we want to perform an action on it.
We can take this loaded data set and **assign** it to a **variable**.
We can do this with the **assignment** **operator**, `<-`.
Note the way it is typed: a less than symbol (`<`) followed immediately by a dash (`-`) without any spaces in between.
...
```{r}
crop <- read_csv("data/crop_recommendation.csv")
```
:::::
...
Notice that this time we no longer see the dataset being printed.
The "Environment" tab in the RStudio panel (on the top right) will now show the loaded dataset with the variable
name we used as an entry. 
Clicking on any listed data set icon in the “Environment” tab will open a view of the dataset in a new window on the left window.
Clicking on the light blue circled arrow next to each loaded data set in the “Environment” tab will show you the column-by-column text representation (technically called the `structure`) of that data set.

To look at our dataset we can execute *just* the variable we assigned the dataset to by either calling the variable in the console window or in an R script.
...
This tabular dataset, which has now been loaded into R, is called a **data frame** **object** (or simply a **dataframe.**),
The `tidyverse` uses a `tibble`object.
...
::::: {.rBlock}
###### R {-}
To read an excel file, we will need a separate library that can handle Excel files, which is `readxl`.
...
```{r}
crop_xl <- read_excel("data/crop_recommendation.xlsx")
```
:::::
...
###### Python {-}
```{python}
crop_xl = pd.read_excel("data/crop_recommendation.xlsx")
```
:::::

We can view the loaded Excel file just like before.
...
or the `type` function in Python to tell you that.
...
Once you have see a `"tbl_df"` or `"data.frame"` (R) or `<class 'pandas.core.frame.DataFrame'>` (Python) you can be sure that the dataframe processing functions, which we will learn, will work.

**Take away**: Once you load a dataset and end up with either a `data.frame` or `tibble` object, you can continue using
the same data processing methods that we cover in these materials.

<button class="accordion" id="question">Exercise 2 Question</button>
:::{.panel-question}
1. Make sure you have the `ds4es` RStudio project open
...
```{r}
select(crop, temperature)
```

If we want multiple columns (or to re-order columns), we can specify each column which we want to be selected.

```{r}
select(crop, label, temperature, humidity)
```
:::::
...
```{python}
crop[["label", "temperature", "humidity"]]
```
...
A common way `tidyverse` code gets written is using the pipe operator,`%>%`. 
The `%>%` takes the object on the left and passes that into the first argument in the function on the right.
...
So, the above statement is read "crop, then select the temperature column."
...
However when we start chaining multiple data sets together,
it will make the code a little more readable.
:::::

## Filtering Rows

Now that we know how to select columns on our dataframe,
we can learn how to filter the rows.

::::: {.rBlock}
###### R {-}
To filter rows, we can use the `filter` function.
The first argument of the `filter` is the dataframe object (the same pattern as the `select` function),
and the other arguments are statements that return some `TRUE` or `FALSE` value (this is known as a **Boolean** condition).

For example, if we want to filter the `crop` dataset such that the `label` value is `rice``, we can filter the rows using that condition.

```{r}
filter(crop, label == 'rice')
```
...
###### Python {-}
```{python}
crop.loc[crop["label"] == 'rice']
```
:::::

If we want to combine multiple conditions, where, for example, `label` is `rice`` **and** `N` is `88`:
...
Also, we can use the `&` for the **and** operator.
...
###### Python {-}
In pandas,
if you have multiple conditions, you must put each condition around a pair of round brackets, i.e., `( )`.

```{python}
crop.loc[(crop["label"] == 'rice') & (crop["N"] == 88)]
```
:::::
...
###### Python {-}
```{python}
crop.loc[(crop["N"] == 88) | (crop["N"] == 69)]
```
:::::

## Making Subset of Columns and Rows

::::: {.rBlock}
###### R {-}
To make a subset on both columns **and** rows,
call the corresponding subset function one after the other.
Here, we are taking the `crop` dataset, then `select`ing the `label`, `N`, and `temperature` columns, then
`filter`ing the rows where the `temperature` is greater than `23.0`.

```{r}
crop %>%
  select(label, N, temperature) %>%
  filter(temperature > 23.0)
```

This is an example where the pipe operator, i.e.,`%>%`, makes the code a little easier to **read**.
We can "un-pipe" our code. It looks like this:

```{r}
filter(select(crop, label, N, temeperature), temperature > 23.0)
```
:::::

::::: {.pyBlock}
###### Python {-}
The `.loc` notation actually has two parts; it looks like `.loc[rows columns]`.
So, in order to specify rows and columns, we separate how we want to select columns with a comma.

```{python}
crop.loc[crop["temperature"] > 23.0, ["label", "N", "temperature"]]
```
:::::
...
Even if all you do is read in data, filter it, and make a subset dataset for another use case,
...
Let us say we want to work with a filtered version of the `crop` dataset.
First, we can write the code to make a subset of the data we want.

::::: {.rBlock}
###### R {-}
```{r}
filtered <- crop %>%
  select(label, P, N, temperature) %>%
  filter(N == 88 | N == 69)
```
...
###### Python {-}
```{python}
filtered = crop.loc[(crop["N"] == 88) | (crop["N"] == 69), ["label", "P", "N", "temperature"]]
```
...
Then, we can write out the dataframe to a file (in this case a CSV text file),
using the same file pathing string format as we loaded a dataset earlier.

::::: {.rBlock}
###### R {-}
```{r}
write_csv(filtered, "data/crop_filtered.csv")
```
:::::
...
###### Python {-}
```{python}
filtered.to_csv("data/crop_filtered_py.csv")
```
:::::

One thing to be mindful of is that most programs and computer languages do not like having spaces in file names.
In general, you will end up typing more characters that deal with spaces in file names
instead of using an underscore, `_`, or dash, `-`.
Also, as a general rule, keeping file names in lowercase will prevent a lot of potential spelling mistakes.
...
- read_csv() is a function used to read data files in .csv format
- A dataframe is a tabular dataset object
- The %>% operator takes the object on the left, and passes that into the first argument to the function on the right
- The filter function is used to filter rows. It takes a dataframe object as the first argument and boolean condition as other arguments
