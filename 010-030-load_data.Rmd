...
So, the above statement is read "tumor, then select the Size column."
...
If we want to combine multiple conditions, where, for example, `Group` is `1` **and** `Day` is `0`:
...
Here, we are taking the `tumor` dataset, then `select`ing the `Group`, `Day`, and `Size` columns, then
...
Let us say we want to work with a filtered version of the `tumor` dataset.
