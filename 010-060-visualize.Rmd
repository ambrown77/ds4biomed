# Visualization (Intro)

## Introduction {#vis-intro-intro}

In this chapter, we will learn to visualize different types of data. 

Load up the libraries and data.

```{r, echo=TRUE, message=FALSE}
library(tidyverse)
library(ggplot2)
```

[Blood storage dataset description](https://github.com/higgi13425/medicaldata/blob/master/description_docs/blood_storage_desc.pdf)

> [A retrospective cohort study of] 316 men who had undergone radical prostatectomy and received transfusion during or within 30 days of the surgical procedure
> and had available PSA follow-up data.
> The outcome [of interest] was time to biochemical cancer recurrence.
> Study evaluated the association between red blood cells storage duration and biochemical prostate cancer recurrence after radical prostatectomy.
> Specifically, tested was the hypothesis that perioperative transfusion of allogeneic RBCs stored for a prolonged period is associated with earlier biochemical recurrence of prostate cancer after prostatectomy.

```{r}
crop <- read_csv("data/crop_recommendation.csv")
head(crop) # use head to look at the first 6 rows
```

```{r}
class(crop)
```


```{r}
# get just the column names for a dataset
names(crop)
```


## Data Types

While exploring a dataset, you want to know what each variable's role in the analysis will be.

- What is the variable (i.e., column) of interest?
  - response, dependent, y, outcome
- which are your predictor variables?
  - predictor, independent variable, x

For each variable, you want to know what possible values it can take on.

```{r, echo=FALSE, fig.cap="\"Continuous Discrete\" by Allison Horst."}
knitr::include_graphics("https://raw.githubusercontent.com/allisonhorst/stats-illustrations/master/other-stats-artwork/continuous_discrete.png")
```

```{r, echo=FALSE, fig.cap="\"Nominal Ordinal Binary\" by Allison Horst."}
knitr::include_graphics("https://raw.githubusercontent.com/allisonhorst/stats-illustrations/master/other-stats-artwork/nominal_ordinal_binary.png")
```


The type of information a variable holds will dictate the summary statistics you can make,
the visualizations you can create,
and the models you can fit.

Ordinal and discrete variables should be converted into `factor` variables in R.
A `factor` is the R's way of naming a **categorical** variable.
This is different from a **character string**, e.g., a person's name.

```{r}
recurrence_freq <- crop %>%
  group_by(type) %>%
  summarize(count = n())
recurrence_freq
```

## The Grammar of Graphics

```{r, echo=FALSE, fig.cap="The idea behind the grammar of graphics is to decompose graphics into its constitudent layers: data, mapping, statistics, scales, geometries, facets, coordinates, and theme. by Thomas Lin Pedersen."}
knitr::include_graphics("./images/ggplot2/ggplot-grammar_layers.png")
```

## Data + Geometries

ggplot is an open source data visualization package in R based on Grammar of Graphics. It lets us visualize data in different layers. We will look at an example using the blood storage dataset. 

```{r}
# add a data layer
ggplot(data = crop)
```

```{r}
# add a data later with an asthetic mapping
ggplot(data = crop, mapping = aes(x = type))
```


```{r}
# add a geometry layer
# by default the stat layer for geom_bar will count values
ggplot(data = crop, mapping = aes(x = type)) + geom_bar()
```

If we have a pre-calculated set of values, we can to tell `geom_bar` to use the `"identity"` statistic.

```{r}
recurrence_freq
```

```{r}
ggplot(data = recurrence_freq, mapping = aes(x = type, y = count)) +
  geom_bar(stat = "identity")
```

Something to think about: we have highly unbalanced classes.
This might be something to think about when you fit models and only look at blind performance metrics. 
For example, there are 100 patients where 99 are healthy and one is sick. If my model classifies healthy every time, it is still 99% correct.


### Layer Values

If a value does not exist in a particular layer,
ggplot will try to use data from the previous layer.

```{r}
# everything in the base layer
ggplot(data = recurrence_freq, mapping = aes(x = type, y = count)) +
  geom_bar(stat = "identity")
```

```{r}
# move astetic mapping to geom layer
ggplot(data = recurrence_freq) +
  geom_bar(mapping = aes(x = type, y = count), stat = "identity")
```

```{r}
# move data to geom layer
ggplot() +
  geom_bar(data = recurrence_freq, mapping = aes(x = label, y = count), stat = "identity")
```

This means we can add more layers with different data sets if we want to.

## Geometries

### Univariate

A type of data that consists of observations on one type of characteristic is called univariate data

#### Continuous

Let us visualize the blood storage dataset as an example of univariate continuous data using a histogram

```{r}
ggplot(crop, aes(x = temperature)) + geom_histogram()
```

```{r}
ggplot(crop, aes(x = temeperature)) + geom_histogram(bins = 10)
```

### Bivariate

The `TVol` column represents the tumor volume as an ordinal variable along with the following definition for each ordinal value:

- 1 = Low
- 2 = Medium
- 3 = Extensive

However, in the dataset, the tumor volume is encoded as a (discrete) numeric variable, even though it actually represents a categorical variable.
To convert the numeric column (or any column) into a categorical **factor**, we can use the `as.factor` function.

```{r}
# Does not show TVol properly
ggplot(crop) + geom_boxplot(aes(x = type, y = temperature))
```

```{r}
# please a box plot for each value of TVol as a factor
ggplot(crop) + geom_boxplot(aes(x = as.factor(type), y = temperature))
```

Also, we can use a violin plot to better visualize the distribution of the dataset,
instead of using a box plot.

In addition, we can overlay a different geometry on top.

```{r}
ggplot(crop) + 
  geom_violin(aes(x = as.factor(type), y = temperature)) +
  geom_point(aes(x = as.factor(type), y = temperature))
```

```{r}
ggplot(crop) +
  geom_violin(aes(x = as.factor(type), y = temperature)) +
  geom_jitter(aes(x = as.factor(type), y = temperature))
```

We can move around our data layers to save some typing by having
the geometry layer use the same data and mapping layer.

```{r}
ggplot(crop, aes(x = as.factor(type), y = temperature)) +
  geom_violin() +
  geom_jitter()
```

## Other Aesthetic Mappings

We can set other asthetic mappings, e.g., color

- `PVol`: Prostate volume in grams (g)
- `PreopPSA`: Preoperative prostate specification antigen (PSA) in ng/mL
- `sGS`: Surgical Gleason score
  - 1 = Not assigned
  - 2 = No residual disease or score 0-6
  - 3 = Score 7
  - 4 = Score 8-10

```{r}
ggplot(crop) +
  geom_point(aes(x = rainfall, y = temperature, color = type))
```

Again, we have a numeric variable that is really an ordinal categorical variable

```{r}
ggplot(crop) +
  geom_point(aes(x = rainfall, y = temperature, color = as.factor(type)))
```


## Facets

Facets allow us to re-plot the same figure by separate groups.
Think of this as the `group_by` version for plotting.


```{r}
# use facet wrap for a single variable
ggplot(crop) +
  geom_point(aes(x = rainfall, y = temperature, color = as.factor(label))) +
  facet_wrap(~ type)
```

```{r}
# use facet grid for 2 variables
ggplot(crop) +
  geom_point(aes(x = rainfall, y = temperature, color = as.factor(label))) +
  facet_grid(label ~ type)
```

## Themes

Themes are a powerful way of customizing the non-data components of your plot. Let us look at an example using the blood storage dataset. 

```{r}
g <- ggplot(crop) +
  geom_point(aes(x = rainfall, y = temperature, color = as.factor(label)))) +
  facet_grid(label ~ type)
g
```

```{r}
g + theme_minimal()
```

Using ggthemes

```{r}
library(ggthemes)
```

```{r}
g + theme_wsj()
```

```{r}
g + theme_fivethirtyeight()
```


```{r}
g + theme_excel()
```


<button class="accordion" id="question">Exercise 1 Question</button>
:::{.panel-question}
1. Load the cytomegalovirus dataset. Then, assign it to the variable, `cmv`.

```{r, eval=FALSE}
library(medicaldata)
___ <- cytomegalovirus
```

2. Bar chart of the `cmv` response variable

```{r, eval=FALSE}
ggplot(cmv) +
  ____(aes(x = ____))
```

3. bar plot of `prior.transplant`, colored by `cmv` values

```{r, eval=FALSE}
ggplot(____, aes(as.factor(____))) +
  geom_bar(aes(fill = as.factor(____)))
```

4. facet by both `donor.cmv` and `recipient.cmv`

```{r, eval=FALSE}
ggplot(cmv, aes(as.factor(prior.transplant))) +
  geom_bar(aes(fill = as.factor(cmv))) +
  ____(____ ~ ____)
```
:::

<button class="accordion" id="solution">Exercise 1 Solution</button>
:::{.panel-solution}
```{r}
cmv <- medicaldata::cytomegalovirus

ggplot(cmv) +
  geom_bar(aes(x = cmv))

ggplot(cmv, aes(as.factor(prior.transplant))) +
  geom_bar(aes(fill = as.factor(cmv)))

ggplot(cmv, aes(as.factor(prior.transplant))) +
  geom_bar(aes(fill = as.factor(cmv))) +
  facet_grid(donor.cmv ~ recipient.cmv)
```
:::

## Additional Resources

- Ggplot2 reference
  - https://ggplot2.tidyverse.org/reference/index.html
- R Graphics Cookbook
  - 1st edition: http://www.cookbook-r.com/Graphs/
  - 2nd edition: https://r-graphics.org/
- Thomas Lin Pedersen ggplot workshop
  - Slides: https://github.com/thomasp85/ggplot2_workshop
  - Part 1: https://www.youtube.com/watch?v=h29g21z0a68
  - Part 2: https://www.youtube.com/watch?v=0m4yywqNPVY
